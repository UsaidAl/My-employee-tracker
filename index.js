  ...
  while (continueExecution) {
    ...
    switch (action) {
      ...
      case "Add an Employee":
        ...
        const manager_id = await promptEmployeeChoice(
          "Select Manager for new Employee:"
        );
        // Add the new employee to the database
        await database.additionalEmployee(
      ...
      case "Change an Employee Role":
        ...
        const employee_to_update_id = await promptEmployeeChoice(
          "Select an Employee:"
        );
        // Prompt the user to select a new role for the employee
        const new_employee_role_id = await promptRoleChoice();
        // Update the employee's role in the database
        await database.changeEmployeeRole(
          new_employee_role_id,
          employee_to_update_id
        );
        break;

      case "Change an Employees Manager":
        const employee_to_update_manager_id = await promptEmployeeChoice(
          "Select an employee:"
        );
        const manager_to_update_id = await promptEmployeeChoice(
          "Select a new Manager for this employee:"
        );
        await database.changeEmployeeManager(
          employee_to_update_manager_id,
          manager_to_update_id
        );
        break;
      ...
      case "Remove an Employee":
        const employee_to_delete_id = await promptEmployeeChoice(
